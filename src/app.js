...
function saveNow(){ store.savedAt=Date.now();
  ...
  const txt=JSON.stringify(store); kvPut('store',txt); try{ localStorage.setItem(LS,txt); }catch(e){ try{ localStorage.removeItem(LS); }catch(_){ } } }
...
function simulate(){
  ...
  route.steps.forEach((s,i)=>{
    ...
    else if((s.t==='accept'||s.t==='complete'||s.t==='turnin'||s.t==='abandon') && !q) r.warn.push('Quest '+s.q+' isn\u2019t in the Questie database yet: no XP or map pin for it');
    else if(s.t==='accept'){
...
const tileCache=new Map(); let tileFail=false;
function tileImg(src){ let im=tileCache.get(src); if(im){ tileCache.delete(src); tileCache.set(src,im); return im.complete&&im.naturalWidth?im:null; }
  if(tileFail) return null; im=new Image(); im.onload=requestDraw; im.onerror=()=>{ tileFail=true; }; im.src=src; tileCache.set(src,im);
  while(tileCache.size>60){ const k=tileCache.keys().next().value; const x=tileCache.get(k); tileCache.delete(k); x.src=''; } return null; }
const HIRES_S=0.1;
function reliefDraw(g,kind){
  ...
  for(const o of reliefImgs){ const ov=kind==='r'?o.ri:o.wi; if(!ov.complete||!ov.naturalWidth) continue;
    ...
    for(let ty=ty0;ty<=ty1;ty++) for(let tx=tx0;tx<=tx1;tx++){ const k=ty+'_'+tx; if(!set.has(k)) continue;
      ...
      const im=tileImg(`tiles/t${o.m}_${k}_${kind}.webp`);
...
function draw(){
  ...
  ctx.setTransform(DPR,0,0,DPR,0,0);
  drawLabels(); drawPOIs(); drawGhosts(); if(layers.route) drawRoute(); drawQuests();
...
canvas.addEventListener('pointerdown',e=>{ canvas.setPointerCapture(e.pointerId); ptrs.set(e.pointerId,{x:e.offsetX,y:e.offsetY}); moved=false; dragStart={x:e.offsetX,y:e.offsetY,tx:view.tx,ty:view.ty};
...
function endPtr(e){ ptrs.delete(e.pointerId); if(ptrs.size<2) pinch=null; canvas.classList.remove('dragging');
...
function removeStep(i){ pushHistory(); route.steps.splice(i,1); if(cursor>=i) cursor--; refresh(); }
...
function blockAction(a){ const idx=[...selSteps].sort((x,y)=>x-y); if(!idx.length) return;
  ...
  if(a==='del'){ pushHistory(); const del=new Set(idx); const before=idx.filter(i=>i<=cursor).length; route.steps=route.steps.filter((_,i)=>!del.has(i)); cursor=Math.max(-1,cursor-before); selSteps.clear(); refresh(); toast(`Deleted ${idx.length} steps`); return; }
...
async function askMoveAfter(idx){ const v=await askText(`Move the ${idx.length} selected steps to just after step number:`,String(cursor+1)); if(v==null) return; const n=parseInt(v); if(!(n>=0&&n<=route.steps.length)) return toast('Enter a step number between 0 and '+route.steps.length);
...
$('#steps').addEventListener('dblclick',e=>{ const li=e.target.closest('[data-i]'); if(!li) return; const s=route.steps[+li.dataset.i]; if(s&&['grind','travel','custom'].includes(s.t)) openStepDialog(s.t,+li.dataset.i); });
let dragFrom=null;
...
const SOURCE_URL=''; // set to the project's GitHub address when it is published
...
function buildRXP(){
  ...
  route.steps.forEach((s,i)=>{
    ...
    if(raw&&gsx&&gsx.rx&&raw[gsx.rx-1]!=null){
      ...
      const runActs=new Set(run.filter(x=>x.q).map(x=>actKey(x.t,x.q))); const blkActs=new Set(blockSteps(gg,gsx.rx).filter(x=>x.cond&&x.q).map(x=>actKey(x.t,x.q)));
...
$('#delRoute').addEventListener('click',async()=>{ if(!await ask(`Delete the route "${route.name}" and its imported guides? This can't be undone.`)) return;
  store.routes=store.routes.filter(r=>r!==route); if(!store.routes.length) store.routes.push(newRoute({name:'New route'}));
...
function buildGuideSteps(g,convert){
  ...
  g.blocks.forEach((b,bi)=>{
    const bOK=condOK(b.cond)&&!b.ver; let loc=null, firstLoc=null; const pend=[]; const notes=[]; const seenC=new Set(); const dg=[], dgs=[]; let lmin=0, lmax=0; const startN=steps.length; const disp=[]; const reqs=[]; let gotoTxt='';
    for(const line of b.lines){
      ...
      if((m=cmd.match(/^\.goto\s+(\d+)\/(\d+)\s*,\s*(-?[\d.]+)\s*,\s*(-?[\d.]+)/i))){ if(!ok) continue; // "uiMapId/continent, worldY, worldX" (Forever guides)
        ...
        loc=convertLoc(z,px,py,false); if(!firstLoc) firstLoc=loc; for(const p of pend) if(!p.loc) p.loc=loc; continue; }
      if((m=cmd.match(/^\.goto\s+([^,]+),\s*([\d.]+)\s*,\s*([\d.]+)/i))){ if(!ok) continue; const z=zoneFromName(m[1]); if(z==null){ unknownZones.add(m[1].trim()); continue; } loc=convertLoc(z,+m[2],+m[3],convert); if(!firstLoc) firstLoc=loc; for(const p of pend) if(!p.loc) p.loc=loc; continue; }
...
function kvPut(k,v){ rawDB().then(db=>{ if(!db) return; try{ db.transaction('kv','readwrite').objectStore('kv').put(v,k); }catch(e){} }); }
...
function importGuides(text,convert){
  ...
  for(const pg of parsed){
    ...
    if(ex){ // same guide again: swap in the new parse and re-point your route's steps at it, keeping their order
      ...
      ex.excluded=(ex.excluded||[]).map(i=>map[i]).filter(i=>i>=0); ex.from=Math.max(0,map[ex.from]??0); ex.steps=steps; ex.to=steps.length-1; refreshed.push(ex); continue; }
...
function ghostToStep(g,i){ const s=g.steps[i]; const o={t:s.t,src:{g:g.id,i}}; for(const k of ['q','mode','level','xp','note','kind','text','counts','gopt','reqs','rx','qn','ride','cw','sticky','label']) if(s[k]!=null) o[k]=s[k]; if(s.loc) o.loc=s.loc; if(s.t==='grind') o.src.xpcheck=true; return o; }
...
function fillGuide(g){
  ...
  if(!pos.size) return 0; const idx=[...pos.keys()].sort((a,b)=>a-b); let lo=idx[0], hi=idx[idx.length-1];
  ...
  const other=new Set(route.steps.filter(s=>s.q&&!(s.src&&s.src.g===g.id)).map(s=>s.t+':'+s.q)); const excl=new Set(g.excluded||[]); let added=0; pushHistory();
  for(let i=lo;i<=hi;i++){ if(pos.has(i)) continue; const s=g.steps[i];
    if(!s.cond||!xrOK(s.xr)||!dungeonOK(s).ok||excl.has(i)) continue; if(s.info&&!g.includeNotes) continue; if(s.t==='grind'&&!g.includeXp) continue;
  ...
  if(!added) history.pop(); return added; }
...
function askText(msg,def){ return new Promise(res=>{ const d=$('#dlgAsk'), inp=$('#askIn'); $('#askMsg').textContent=msg; $('#askOk').textContent='Move'; inp.hidden=false; inp.value=def||'';
